...
- **AgileEVM** (`src/agileEvm.ts`, `AgileEvmView.tsx`, the "EVM" nav tab) — earned value management adapted to Scrum per Sulaiman & Baham's method: story points stand in for the dollar-denominated budget. Needs one new optional `ProjectConfig` field, `plannedSprints` (a release-length schedule baseline); the view stays in a "set a baseline" state until it's provided. Computes PV (planned value, linear against the baseline), EV (cumulative points completed), SPI (`EV/PV`), and a schedule-derived finish forecast (`plannedSprints / SPI`). Deliberately schedule-only — no CPI/EAC/ETC: with cost modeled as a flat rate per sprint (the only cost input this app could ask for without new per-sprint tracking), Actual Cost is exactly proportional to elapsed sprints, same as PV, so `CPI = EV/AC` collapses to `EV/PV = SPI` algebraically every time — not a second signal, SPI wearing a cost label. GOAL.md is explicit that this app never presents a metric without the context to read it honestly, so the cost side is left for a follow-up once sprints can carry a genuine per-sprint actual-cost field that can vary independently of the plan.
