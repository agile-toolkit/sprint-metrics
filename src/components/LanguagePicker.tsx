  ...
  return (
    <div ref={ref} className="relative" onKeyDown={handleKeyDown}>
      <button
        ...
        aria-expanded={open}
        className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-100 transition-colors select-none dark:text-gray-400 dark:hover:bg-gray-800"
      >
      ...
      {open && (
        <ul
          role="listbox"
          aria-label="Language"
          className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg py-1 z-50 min-w-[110px] dark:bg-gray-900 dark:border-gray-700"
        >
          {LANGUAGES.map(lang => {
              ...
              >
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-1.5 text-sm transition-colors ${
                    isCurrent
                      ? 'bg-brand-50 text-brand-700 font-semibold dark:bg-brand-700/20 dark:text-brand-400'
